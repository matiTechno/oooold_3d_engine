...
uniform sampler2D mat_tex_diffuse0;//uniform vec3 mat_diffuse;
uniform sampler2D mat_tex_specular0;//uniform vec3 mat_specular;
...
vec3 add_sunLight(SunLight light, vec3 viewDir, vec3 normal)
{
	...
	vec3 ambient = vec3(texture(mat_tex_diffuse0, texCoord0)) * light.ambient;
	vec3 diffuse = vec3(texture(mat_tex_diffuse0, texCoord0)) * light.diffuse * diff_coeff * (1 - shadow);
	vec3 specular = vec3(texture(mat_tex_specular0, texCoord0)) * light.specular * spec_coeff * (1 - shadow);
...
vec3 add_pointLight(PointLight light, vec3 viewDir, vec3 normal)
{
	...
	vec3 ambient = vec3(texture(mat_tex_diffuse0, texCoord0)) * light.ambient * attenuation;
	vec3 diffuse = vec3(texture(mat_tex_diffuse0, texCoord0)) * light.diffuse * diff_coeff * attenuation;
	vec3 specular = vec3(texture(mat_tex_specular0, texCoord0)) * light.specular * spec_coeff * attenuation;
...
vec3 add_spotLight(SpotLight light, vec3 viewDir, vec3 normal)
{
	...
	vec3 ambient = vec3(texture(mat_tex_diffuse0, texCoord0)) * light.ambient * attenuation;
	vec3 diffuse = vec3(texture(mat_tex_diffuse0, texCoord0)) * light.diffuse * diff_coeff * attenuation * spot_coeff;
	vec3 specular = vec3(texture(mat_tex_specular0, texCoord0)) * light.specular * spec_coeff * attenuation * spot_coeff;
